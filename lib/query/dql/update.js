    ...
    return parley(
        function (done) {
            ...
            (function (proceed) {
                ...
                WLModel._callbacks.beforeUpdate(query.valuesToSet, tableName, function (err) {
                    if (err) {
                        return proceed(err);
                    }
                    return proceed(undefined, query);
                ...
                adapter.find(WLModel.datastore, query, function (err, findRecord) {
                        ...
                        async.each(_.keys(collectionResets), function _eachReplaceCollectionOp(collectionAttrName, next) {
                            ...
                            function _afterReplacingAllCollections(err) {
                                ...
                                async.each(transformedRecords, function _eachRecord(record, next) {
                                        ...
                                        // added code by parth starts
                                        let fRecord = _.find(findRecord, function (r) {
                                            return r._id === record.id
                                        });
                                        fRecord.id = fRecord._id
                                        delete fRecord._id;
                                        // added code by parth ends
                                        // Otherwise run it.
                                        WLModel._callbacks.afterUpdate(record, fRecord, tableName, query.valuesToSet, function _afterMaybeRunningAfterUpdateForThisRecord(err) {
                                            if (err) {
                                                return next(err);
                                            }
