    ...
    return parley(
        function (done) {
            ...
            (function _runBeforeLC(proceed) {
                ...
                WLModel._callbacks.beforeDestroy(query.criteria, tableName, function (err) {
                    if (err) {
                        return proceed(err);
                    }
                    return proceed(undefined, query);
                        ...
                        (function _maybeWipeAssociatedCollections(proceed) {
                            ...
                            (function _maybeTransformRecords(proceed) {
                                ...
                                async.each(transformedRecordsMaybe, function _eachRecord(record, next) {
                                        ...
                                        // Otherwise run it.
                                        WLModel._callbacks.afterDestroy(record, tableName, function _afterMaybeRunningAfterDestroyForThisRecord(err) {
                                            if (err) {
                                                return next(err);
                                            }
